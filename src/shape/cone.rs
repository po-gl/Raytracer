...
impl Cone {
    pub fn new() -> Cone {
        ...
        Cone {id, transform: Matrix4::identity(), material: Material::new(), minimum: NumFloat::neg_infinity(), maximum: NumFloat::infinity(), closed: false}
    }

    pub fn new_with_material(material: Material) -> Cone {
        let id = shape::get_shape_id();
        Cone{id, transform: Matrix4::identity(), material, minimum: NumFloat::neg_infinity(), maximum: NumFloat::infinity(), closed: false}
    }

    pub fn new_bounded(minimum: f64, maximum: f64) -> Cone {
        let id = shape::get_shape_id();
        Cone {id, transform: Matrix4::identity(), material: Material::new(), minimum, maximum, closed: false}
    }
